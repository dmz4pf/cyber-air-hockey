...
export function useLinera(options: UseLineraOptions = {}): UseLineraReturn {
  ...
  const submitResult = useCallback(
    async (gameId: number, player1Score: number, player2Score: number) => {
      ...
      setIsLoading(true);
      try {
        // Signature is handled internally by the client
        // ProductionLineraClient uses MetaMask to sign
        // MockLineraClient generates a mock signature
        await client.current.submitResult({
          gameId,
          player1Score,
          player2Score,
          signature: '', // Signature generated by client
        });
      } catch (err) {
